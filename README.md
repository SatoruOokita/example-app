...
ターミナルでVagrantfileが存在するディレクトリへ移動しておきましょう。
...
### 2-2. 仮想マシンにログインする 
仮想マシンが立ち上がっているか確認します。

    vagrant status

「running」と表示されたらssh接続でログインします。
...
### 2-5. .envファイルの編集 
.env.exampleの19行目~25行目にデプロイ環境でのmysqlの設定をコメントアウトして記載しています。
そのコメントアウトを解除した後、11行目~17行目に書いている開発環境でのmysqlの設定を削除しておいてください。
